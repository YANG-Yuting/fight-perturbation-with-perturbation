        ...
        for j in perturb_idx:
            ...
            if isinstance(word, int) and pos in args.pos_list:
                if pos.startswith('JJ'):
                    ...
                    pos = 'verb'
                neigbhours_list.append(args.word_candidate[word][pos].copy())
            else:
                neigbhours_list.append([])
        neigbhours_list = [[args.inv_full_dict[i] if isinstance(i, int) else i for i in position] for position in neigbhours_list]
...
def main():
    wrong_clas_id = []  # the id of data which is predicted wrong
    wrong_clas = 0  # the num of data which is predicted wrong
    failed_list = []  # the id of data which is attacked failed
    failed_time = []  # time
    failed_input_list = []  # the data which is attacked failed
    # For successful attack:
    input_list = []  # the data which is attacked successfully
    output_list = []  # the adversarial examples
    success = []  # id
    change_list = []  # change ratio
    true_label_list = []  # gold label
    success_count = 0  # num
    num_change_list = []  # change num
    success_time = []  # time
    ...
    # train set
        ...
        texts = [[args.inv_full_dict[word] for word in text]for text in texts]
