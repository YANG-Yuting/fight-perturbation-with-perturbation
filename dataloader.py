...
import numpy as np
import torch
from pad_setting import task, mr_pad_len, imdb_pad_len
...
def read_corpus(path, clean=True, FAKE=True, encoding='utf8', shuffle=False, lower=True):
    data = []
    labels = []
    with open(path, encoding=encoding) as fin:
        for line in fin:
            ...
            if not FAKE:
                ...
                # label, sep, text = line.partition(',')
                # label = int(label) - 1
                label = int(line[-1]) - 1  # tiz: original ways to read is wrong
                text = line[:-2]
            if clean:
...
def pad(sequences, max_len, pad_token='<pad>', pad_left=True):
    ''' input sequences is a list of text sequence [[str]]
        pad each text sequence to the length of the longest
    '''

    # 原始代码是每个batch按照最长的那条数据pad --> 我们改为所有数据按照统一的长度pad
    # 该方法pad，fake测试集上准确率在63.7%
    # max_len = max(5, max(len(seq) for seq in sequences))
    # if pad_left:
    #     return [ [pad_token]*(max_len-len(seq)) + seq for seq in sequences ]

    # 该方法pad，max_len=512时，fake测试集上准确率在60.5%；max_len==2000时，fake测试集上准确率在63.6%
    if pad_left:
        pad_sequences = []
        for seq in sequences:
            if len(seq) > max_len:
                pad_sequences.append(seq[:max_len])
            else:
                pad_sequences.append([pad_token] * (max_len - len(seq)) + seq)
        return pad_sequences


def create_one_batch(x, y, max_len, map2id, oov='<oov>'):
    oov_id = map2id[oov]
    x = pad(x, max_len)
    length = len(x[0])
    batch_size = len(x)
...
def create_one_batch_x(x, max_len, map2id, oov='<oov>'):
    oov_id = map2id[oov]
    x = pad(x, max_len)
    length = len(x[0])
    batch_size = len(x)
...
def create_batches(x, y,max_len, batch_size, map2id, perm=None, sort=False, weight=None):
    ...
    if weight!=None:
        weight=[weight[i] for i in lst] # tiz
    ...
    # nbatch = nbatch -1 # 丢掉最后一个batch
    for i in range(nbatch):
        bx, by = create_one_batch(x[i * size:min((i + 1) * size,len(x))], y[i * size:min((i + 1) * size,len(x))], max_len, map2id)
        batches_x.append(bx)
        batches_y.append(by)

        if weight != None:
            batches_weight.append(torch.tensor(weight[i * size:min((i + 1) * size,len(x))]))
    ...
    if sort:
        ...
        if weight != None:
            batches_weight = [ batches_weight[i] for i in perm ]
    ...
    # ))
    if weight!=None:
        return batches_x, batches_y, batches_weight
    else:
        return batches_x, batches_y


# shuffle training examples and create mini-batches
def create_batches_x(x, max_len, batch_size, map2id, perm=None, sort=False):
    ...
    nbatch = (len(x)-1) // size + 1
    for i in range(nbatch):
        bx = create_one_batch_x(x[i*size:min((i + 1) * size,len(x))], max_len, map2id)
        sum_len += len(bx)
        batches_x.append(bx)
