...
def train_model2(args, epoch, model, optimizer,train_data, best_test, save_path):
    ...
    for step in range(0, len(train_data['s2']), args.batch_size):
        ...
        Samples_x=gen_sample_multiTexts(args, None, train_s2, sample_size,0.4)   #sample_size for each point
...
def trainepoch(epoch,model,optimizer,train_dataloader):
    ...
    print('results : epoch %d ; mean accuracy train : %f; loss=%f'% (epoch, train_acc,sumloss))
    return train_acc
...
def main():
    ...
    if args.target_model == 'wordLSTM':
        ...
        optimizer = optim.Adam(model.nli_net.parameters(), lr=1e-5)
        print("Model built!")
